...
function numeroMasGrande(arrayOfNums) {
   ...
   var numeroGrande=0;
   for (var i = 0; i < arrayOfNums.length; i++){
      if (arrayOfNums[i] > numeroGrande) {numeroGrande = arrayOfNums[i];
      }
   }
...
function empiezaConNueve(num) {
   ...
   if (numeroPartido[0] == 9){return true;}
   else  {return false;}
}
...
function todosIguales(array) {
   ...
   if (array[i] !== array[0]){
      return false;
   } else if(array[i] === array[0]) {return true;}
}
...
function breakStatement(num) {
   ...
   while(i<10){
      ...
      if(valor==i) {return "Se interrumpió la ejecución"
      break
   }
   i++;
} return nuevoArray;
